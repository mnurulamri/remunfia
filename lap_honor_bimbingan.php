...
if ($_SESSION["username"]=="admin" or $_SESSION["username"]=="remunerasifisipui" or $_SESSION["username"]=="zaenal"){
	$kd_organisasi = $_POST["kode_prodi"];
...
require_once('/home/ppaa/public_html/tcpdf/tcpdf_include.php');
...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$this->Image($image_file, 17, 5, 15, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);//(file,x,y,w,h,type,link,align,resize,dpi,palign,ismask,imgmask,border,fitbox,hidden,fitonpage,alt,altimgs)
		// Set font
...
// set margins
$pdf->SetMargins ('18',PDF_MARGIN_TOP,'10'); //(PDF_MARGIN_LEFT, PDF_MARGIN_TOP, PDF_MARGIN_RIGHT);
...
$query = "SELECT nama_pengajar, jenis_bimbingan, jml_mhs, harga_satuan, honor 
		FROM bimbingan a, master_pengajar b
		WHERE a.nip = b.nip and a.kd_organisasi = '$kd_organisasi' and
			  a.tahun = $tahun and a.bulan = '$bulan'
...
foreach($data as $pengajar => $records){
	...
	foreach($records as $baris){
		...
		$html.= '<td style="border-top:1px solid gray; border-bottom:1px solid gray;width:15%; text-align:right">'.number_format($baris->harga_satuan).'</td>';
...
$html.= '<br><br>';
$html.= '<table>';
$html.= '<tr nobr="true"><td style="width:60%;"></td><td style="width:40%; font-weight:bold;">Depok, '.DateToIndo().'<br>Mengetahui,<br>Ketua '.$_SESSION['programstudi'].'<br><br><br><br><br>'.$_SESSION['ketuaprogram'].'<br>NIP/NUP: '.$_SESSION['nip'].'</td></tr>';
$html.= '</table>';
